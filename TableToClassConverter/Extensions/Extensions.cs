...
namespace TableToClassConverter
{
    internal static class StandardExtensions
    {
        ...
        public static char FlipCap(this char c)
        {
            var v = c.ToString();
            var v2 = c.ToString().ToUpper();
            
            return v == v2 ? v.ToLower()[0] : v.ToUpper()[0];
            
        }

        public static string ReplaceAllWith(this string source, string toReplace, char replaceWith)
        {
            var res = source;
            foreach(var ch in toReplace)
            {
                res = res.Replace(ch, replaceWith);
            }
            return res;
        }
